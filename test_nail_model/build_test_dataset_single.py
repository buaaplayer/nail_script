...
def compare_ious(pre_ann,gt_ann,gt_shapes,low_iou_count):
    ...
    return kps_list,raw_kps_list,low_iou_count,low_iou_flag
...
def create_compare_pair(pre_json_path,gt_json_list):
    ...
    for i in range(len(pre_img_names)):
        ...
        single_kps_pair, raw_kps_list,low_iou_count,low_iou_flag= compare_ious(pre_ann, gt_ann, gt_shape,low_iou_count)
        if low_iou_flag:
            low_iou_img_name.append(name)
        # print(name)
        all_kps_pair.extend(single_kps_pair)
        all_raw_kps_list.extend(raw_kps_list)
    print("low_iou_count:",low_iou_count)
    return all_kps_pair,all_raw_kps_list,low_iou_img_name
...
def evalution(pre_json_path,gt_json_list,txt_path=None):
    if txt_path:
        _,gt_json_list=parse_pred_txt(txt_path)
    all_kps_pair,all_raw_test_gt_list,low_iou_img_name=create_compare_pair(pre_json_path,gt_json_list)
    nail_count=0
    nail_sqrt_sum=0
    max_loss=0
    max_cout=0
    for i in range(len(all_kps_pair)):
        ...
        nail_count+=1
    with open("low_iou_name.txt",'w') as f:
        f.write("======================\n")
        f.write("nail_count:"+str(nail_count)+"\n")
        f.write("pandiu+low_iou_count:"+str(4072-nail_count)+"\n")
    ...
    print("pandiu+low_iou_count:",4072-nail_count)
...
if __name__ == '__main__':
    test_path="/home/SENSETIME/wangpengju/github/data/tracking_data_test/result_rpn_nobn_kp200w_pd200w_kpgtx2.caff.txt"
...
#for evalution
    ...
    evalution(pre_json,gt_json_list)
    # test()